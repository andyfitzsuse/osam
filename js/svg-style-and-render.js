...
var onDownloadPngClick = function(svg) {
  return function(e) {
    ...
    img.onload = function() {
      ...
      a.download = (svg.id || 'filename-goes-here') + '.png';
      a.click();
...
var onDownloadSvgClick = function(svg) {
  return function(e) {
    var targetSize = getTargetSizeFromClientRect(svg.getBoundingClientRect(), 768, 768);
    var data = serializeSvg(svg, targetSize.width + 'px', targetSize.height + 'px');
    var win = window.URL || window.webkitURL || window;
    ...
    var url = win.createObjectURL(blob);
    e.target.href = url;
    e.target.download = (svg.id || 'filename-goes-here') + '.svg';
  }
}
